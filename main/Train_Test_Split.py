...
num = [500, 1000]
ctypes = ["hESC", "hHEP", "mDC", "mESC", "mHSC-E", "mHSC-GM", "mHSC-L"]
types = ["Specific"]
...
for i in num:
  for j in ctypes:
   for k in types:
    ...
    parser.add_argument('--num', type=int, default= i, help='network scale')
    parser.add_argument('--p_val', type=float, default=0.5, help='the position of the target with degree equaling to one')
    parser.add_argument('--data', type=str, default=j, help='data type')
    parser.add_argument('--net', type=str, default=k, help='network type')
    ...
    if __name__ == '__main__':
        ...
        data_dir = '/auto/k2/aykut3/GENELink/Dataset/Benchmark Dataset/'
        
        density = Network_Statistic(data_type=data_type, net_scale=args.num, net_type=net_type)
    
        TF2file = data_dir + net_type + ' Dataset/' + data_type + '/TFs+' + str(args.num) + '/TF.csv'
        Gene2file = data_dir + net_type + ' Dataset/' + data_type + '/TFs+' + str(args.num) + '/Target.csv'
        label_file = data_dir + net_type + ' Dataset/' + data_type + '/TFs+' + str(args.num) + '/Label.csv'
